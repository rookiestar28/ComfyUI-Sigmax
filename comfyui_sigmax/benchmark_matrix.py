...
def _validate_row(value: object) -> tuple[str, str]:
    ...
    if profile["id"] not in {"krea2.raw.official", "krea2.turbo.official"}:
        raise ScheduleContractError("benchmark profile is unsupported")
    if profile["variant"] not in {"RAW", "Turbo"} or profile["version"] != "1":
...
def _validate_matrix(value: object) -> dict[str, object]:
    ...
    if len(results) != 23 or identities != sorted(identities) or len(set(identities)) != 23:
